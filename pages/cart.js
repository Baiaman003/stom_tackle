...
const Cart = () => {
  ...
  useEffect(() => {
    const cartLocal = JSON.parse(localStorage.getItem('__next__cart01__baygroup'))
  ...
  const handlePayment = async () => {
    if (!address || !mobile)
      return dispatch({
        type: 'NOTIFY',
        payload: { error: 'Пожалуйста, добавьте свой адрес и мобильный.' },
      })
    ...
    if (newCart.length < cart.length) {
      ...
      return dispatch({
        type: 'NOTIFY',
        payload: {
          error: 'Товара нет в наличии или его недостаточно.',
        },
      })
    }
  ...
  return (
    <div className='row mx-auto'>
      ...
      <div className='col-md-4 my-3 text-right text-uppercase text-secondary'>
        ...
        <h3>
          Итого: <span className='text-danger'>{total} сом</span>
        </h3>
